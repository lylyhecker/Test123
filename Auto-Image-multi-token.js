; (async () => {
  ...
  const CONFIG = {
    ...
    THEMES: {
      "Light Glass": {
        primary:"#f7f9fc", secondary:"#ffffff", accent:"#eaf0ff", text:"#0b1020",
        highlight:"#5a86ff", success:"#16a34a", error:"#ef4444", warning:"#f59e0b",
        fontFamily:"'Inter', 'Segoe UI', Roboto, sans-serif",
        borderRadius:"14px", borderStyle:"solid", borderWidth:"1px",
        boxShadow:"0 10px 30px rgba(0,0,0,.05), 0 0 0 1px rgba(10,20,40,.06)",
        backdropFilter:"blur(14px)",
        animations:{ glow:false, scanline:false, pixelBlink:false }
      },
      "Midnight Glass": {
        primary:"#0b1020", secondary:"#11162a", accent:"#121a33", text:"#e7ecf7",
        highlight:"#7aa2ff", success:"#22c55e", error:"#ef4444", warning:"#f59e0b",
        fontFamily:"'Inter', 'Segoe UI', Roboto, sans-serif",
        borderRadius:"14px", borderStyle:"solid", borderWidth:"1px",
        boxShadow:"0 10px 40px rgba(0,0,0,.35), 0 0 0 1px rgba(255,255,255,.06)",
        backdropFilter:"blur(10px)",
        animations:{ glow:true, scanline:false, pixelBlink:false }
      },
      "Matrix Neon": {
        primary:"#0a0f0a", secondary:"#0f1510", accent:"#0d1c12", text:"#c9fdd7",
        highlight:"#00ff41", success:"#39ff14", error:"#ff073a", warning:"#ffff00",
        fontFamily:"'IBM Plex Mono', monospace",
        borderRadius:"12px", borderStyle:"solid", borderWidth:"2px",
        boxShadow:"0 0 18px rgba(0,255,65,.16), inset 0 0 16px rgba(0,255,65,.08)",
        backdropFilter:"blur(0px)",
        animations:{ glow:true, scanline:true, pixelBlink:true }
      },
  ...
  class TokenPool {
    ...
    _isValid(t){ return t && (t.exp - (CONFIG.TOKEN?.EXPIRE_SKEW_MS||15000)) > Date.now(); }
    add(token, ttlSec=120, meta={}){
      const exp = Date.now() + ttlSec*1000;
      this.list.push({ token, exp, lastUsedAt: 0, meta });
    }
    bulkImport(lines){
      (lines||'').split(/\r?\n/).map(s=>s.trim()).filter(Boolean).forEach(line=>{
        const [tok, ttl] = line.split(',').map(s=>s.trim());
        this.add(tok, Number(ttl)>0?Number(ttl):120, {manual:true});
      });
    }
    sweep(){ const n=Date.now(); this.list = this.list.filter(t=>this._isValid(t)); }
    stats(){
      const n=Date.now();
      const valid = this.list.filter(t=>this._isValid(t));
      return { total:this.list.length, valid:valid.length, expiresInSec: valid.map(t=>Math.max(0, Math.floor((t.exp-n)/1000))) };
    }
    _nextIndex(){
      if (!this.list.length) return -1;
      for (let i=0;i<this.list.length;i++){
        this._rotIndex = (this._rotIndex + 1) % this.list.length;
        if (this._isValid(this.list[this._rotIndex])) return this._rotIndex;
      }
      return -1;
    }
    async get(getGenerator){
      // try pool first
      ...
      // generator allowed?
      if (this.source !== 'manual' && typeof getGenerator==='function'){
        try{
          const {token, ttlSec=120} = await getGenerator();
          if (token){ this.add(token, ttlSec, {manual:false}); return token; }
        }catch(e){ console.warn('Token generator failed', e); }
      }
      return null;
    }
  }
...
// Optimized Turnstile token handling with caching and retry logic
  ...
  async function ensureToken() {
    // try to get from pool (rotating)
    const token = await state.tokenPool.get(async ()=>{
      // generator callback uses existing handleCaptchaWithRetry
      const t = await handleCaptchaWithRetry();
      // we don't know exact TTL; use 240s default (TOKEN_LIFETIME/1000 if present)
      return { token: t, ttlSec: 240 };
    });
    if (token){
      turnstileToken = token;
      tokenExpiryTime = Date.now() + (CONFIG.TOKEN?.EXPIRE_SKEW_MS ? (CONFIG.TOKEN.EXPIRE_SKEW_MS + 1000) : 15000) + 200000; // approximate
  ...
  async function detectLanguage() {
    ...
    // Bind Token Pool UI if present
    const bulkInput = settingsContainer.querySelector('#tokenBulkInput');
    const btnImport = settingsContainer.querySelector('#btnImportTokens');
    const btnStats = settingsContainer.querySelector('#btnPoolStats');
    ...
    if (tokenSourceSelect){
      tokenSourceSelect.addEventListener('change', (e)=>{
        ...
        if (state.tokenPool) state.tokenPool.setSource(state.tokenSource);
      });
    }
    if (btnImport && bulkInput){
      btnImport.addEventListener('click', ()=>{
        state.tokenPool?.bulkImport(bulkInput.value || '');
        bulkInput.value='';
        const s = state.tokenPool?.stats();
        const el = settingsContainer.querySelector('#tokenPoolStats');
        if (s && el){ el.textContent = `total=${s.total}, valid=${s.valid}, exp(s)=[${s.expiresInSec.join(',')}]`; }
      });
    }
    if (btnStats){
  ...
  const injectThemeCSS = () => {
    const theme = getCurrentTheme ? getCurrentTheme() : CONFIG.THEMES[CONFIG.currentTheme] || {};
    const s = document.createElement('style');
    s.dataset.wplaceTheme = "true";
    s.textContent = `
      .wplace-card { background:${theme.accent || 'rgba(0,0,0,.2)'}; color:${theme.text || '#fff'};
        border:${theme.borderWidth || '1px'} ${theme.borderStyle || 'solid'} rgba(255,255,255,.08);
        border-radius:${theme.borderRadius || '12px'}; box-shadow:${theme.boxShadow || 'none'};
        backdrop-filter:${theme.backdropFilter || 'blur(6px)'}; font-family:${theme.fontFamily || 'sans-serif'}; }
      .wplace-btn-primary { ${CONFIG.CSS_CLASSES?.BUTTON_PRIMARY || ''} }
      .wplace-btn-secondary { ${CONFIG.CSS_CLASSES?.BUTTON_SECONDARY || ''} }
      .wplace-grad-text { ${CONFIG.CSS_CLASSES?.GRADIENT_TEXT || ''} }
      .wplace-label { color:${theme.text || '#fff'}; opacity:.8 }
    `;
    ...
    // remove old theme style and inject new
    const oldStyle = document.querySelector('style[data-wplace-theme="true"]'); if (oldStyle) oldStyle.remove();
    injectThemeCSS();

    loadThemePreference()
    loadLanguagePreference()

    if (!state.tokenPool){ state.tokenPool = new TokenPool({source: state.tokenSource || CONFIG.TOKEN_SOURCE}); }
  ...
  .resize-canvas-stack { position: relative; transform-origin: center center; display: inline-block; }
    ...
    settingsContainer.innerHTML = `
      ...
      <div style="padding: 25px; max-height: 70vh; overflow-y: auto;">
        ...
        <!-- Token Pool Controls -->
        <div style="margin-bottom: 25px;">
          <label style="display:block; margin-bottom:12px; color:${theme.text || '#fff'}; font-size:16px; display:flex; align-items:center; gap:8px;">
            <i class="fas fa-database" style="color:${theme.highlight || '#4facfe'}; font-size:16px;"></i>
            Token Pool
          </label>
          <textarea id="tokenBulkInput" rows="4" placeholder="Paste tokens here, one per line (optional: token,ttlSec)"
            style="width:100%; border-radius:8px; border:1px solid rgba(255,255,255,.2); background:rgba(0,0,0,.15); color:${theme.text || '#fff'}; padding:10px;"></textarea>
          <div style="display:flex; gap:8px; margin-top:10px">
            <button class="wplace-btn" id="btnImportTokens" style="background:${theme.highlight || '#4facfe'}; color:#000">Import</button>
            <button class="wplace-btn" id="btnPoolStats">Stats</button>
          </div>
          <div id="tokenPoolStats" style="margin-top:8px; font-family:monospace; opacity:.85"></div>
        </div>
