; (async () => {
  ...
  const CONFIG = {
    ...
    TOKEN: {
      CONCURRENCY: 3,          // max simultaneous token usage / parallel requests
      PREFETCH: 6,             // number of tokens to keep warm in the pool
      RETRY_LIMIT: 4,          // per-batch retry count on token/temp errors
      RETRY_BASE_DELAY_MS: 600,// backoff base in ms
      EXPIRY_SKEW_MS: 25000,   // treat tokens as expired a bit early
      SUBBATCH_SIZE: 200       // pixels per sub-batch request (keeps payloads light)
    },
...
;class TokenPool {
    constructor(genFn, opts) {
      ...
      this._expirySkew = opts?.expirySkew ?? 25000;
      this._pool = []; // {token, expiresAt, inUse:false}
      this._waiters = [];
    ...
    _isValid(t) {
      if (!t) return false;
      const now = Date.now();
      return t.expiresAt && (t.expiresAt - this._expirySkew) > now;
    }
    async _refill() {
      if (this._refilling) return;
      this._refilling = true;
      try {
        while (this._pool.length < this._prefetch) {
          const tok = await this._genFn();
          if (this._isValid(tok)) this._pool.push({ ...tok, inUse: false });
          else break;
        }
        // wake waiters if possible
        while (this._waiters.length && this._pool.find(t => !t.inUse && this._isValid(t))) {
          const waiter = this._waiters.shift();
          waiter?.resolve(this._takeOne());
        }
      } finally {
        this._refilling = false;
      }
    ...
    _takeOne() {
      for (const t of this._pool) {
        if (!t.inUse && this._isValid(t)) {
          t.inUse = true;
          return { token: t.token, release: () => { t.inUse = false; } };
        }
      }
      return null;
    }
    async get() {
      // cleanup
      this._pool = this._pool.filter(t => this._isValid(t));
      const got = this._takeOne();
      if (got) { this._refill(); return got; }
      this._refill();
      return new Promise((resolve) => this._waiters.push({ resolve }));
    }
    invalidate(deadToken) {
      this._pool = this._pool.filter(t => t.token !== deadToken);
      this._refill();
    }
...
;class PixelQueue {
    ...
    next() {
      if (this._idx >= this._list.length) return null;
...
;class PaintScheduler {
    constructor({ tokenPool, concurrency, retryLimit, retryBaseDelay, onPixelDone }) {
      ...
      this.onPixelDone = onPixelDone;
      this._running = false;
      this._abort = new AbortController();
    }
    stop() { this._running = false; try{ this._abort.abort(); }catch{} }
    async placeSubBatchWithRetry(subBatch, regionX, regionY, attempt = 0) {
      const lease = await this.tokenPool.get();
      const token = lease.token;
      try {
        const ok = await sendPixelSubBatchWithToken(subBatch, regionX, regionY, token, this._abort.signal);
        if (!ok) {
          if (attempt < this.retryLimit && this._running) {
            await new Promise(r => setTimeout(r, this.retryBaseDelay * Math.pow(2, attempt)));
            lease.release?.();
            return this.placeSubBatchWithRetry(subBatch, regionX, regionY, attempt + 1);
          }
        } else {
          // count painted
          this.onPixelDone?.(subBatch.length);
        }
      } catch (e) {
        // consider token possibly bad on auth errors
        try {
          if (/401|403/.test(String(e?.status || ''))) this.tokenPool.invalidate(token);
        } catch {}
        if (attempt < this.retryLimit && this._running) {
          await new Promise(r => setTimeout(r, this.retryBaseDelay * Math.pow(2, attempt)));
          lease.release?.();
    ...
    async runSubBatches(subBatches, regionX, regionY) {
      this._running = true;
      while (this._running && subBatches.length) {
        const slots = Math.max(1, Math.min(
          this.concurrency,
          Math.max(1, state.currentCharges || 1)
        ));
        const tasks = [];
        for (let i=0; i<slots && subBatches.length; i++) {
          tasks.push(this.placeSubBatchWithRetry(subBatches.shift(), regionX, regionY));
        }
  ...
  // Adapter: use existing captcha generator to get a token with pseudo expiry
  async function generateTurnstileTokenAdapter() {
    // Reuse existing handleCaptcha() which already respects TOKEN_SOURCE etc.
    const tok = await handleCaptcha();
    // Assume ~110s validity window unless server says otherwise
    return { token: tok, expiresAt: Date.now() + 110_000 };
  }
  // === [/MULTI-TOKEN PATCH] ===
tings: null,
  originalImage: null,
      ...
      outerLoop: for (let y = startRow; y < height; y++) {
        for (let x = y === startRow ? startCol : 0; x < width; x++) {
          ...
          while (state.currentCharges < state.cooldownChargeThreshold && !state.stopFlag) {
            ...
            updateUI("noChargesThreshold", "warni
  // [MULTI-TOKEN PATCH] send sub-batch with explicit token (no global turnstileToken)
  async function sendPixelSubBatchWithToken(pixelBatch, regionX, regionY, token, signal) {
    // Build payload exactly like sendPixelBatch
    const coords = new Uint16Array(pixelBatch.length * 2);
    const colors = new Uint8Array(pixelBatch.length);
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        const err = new Error("auth_error"); err.status = res.status; throw err;
      }
      if (!res.ok) return false;
      const data = await res.json();
      return data?.painted === pixelBatch.length;
    } catch (e) {
      console.warn("[SubBatch] request failed:", e?.message || e);
      return false;
    }
  }
ng", {
  ...
  async function sendBatchWithRetry(pixels, regionX, regionY, maxRetries = MAX_BATCH_RETRIES) {
    // Split into sub-batches and send concurrently using a token pool
    const SUB = Math.max(1, CONFIG.TOKEN.SUBBATCH_SIZE || 200);
    const chunks = [];
    for (let i=0;i<pixels.length;i+=SUB) chunks.push(pixels.slice(i, i+SUB));

    const tokenPool = new TokenPool(
      generateTurnstileTokenAdapter,
      { prefetch: CONFIG.TOKEN.PREFETCH, expirySkew: CONFIG.TOKEN.EXPIRY_SKEW_MS }
    );
    await tokenPool._refill();

    const scheduler = new PaintScheduler({
      tokenPool,
      concurrency: CONFIG.TOKEN.CONCURRENCY,
      retryLimit: CONFIG.TOKEN.RETRY_LIMIT,
      retryBaseDelay: CONFIG.TOKEN.RETRY_BASE_DELAY_MS,
      onPixelDone: (n) => {
        state.paintedPixels += n;
        updateStats?.();
      }
    });
    ...
    while (attempt < maxRetries && !state.stopFlag) {
      attempt++;
      const pending = chunks.slice(); // clone
      await scheduler.runSubBatches(pending, regionX, regionY);

      const paintedNow = state.paintedPixels; // incremental already counted
      // Determine success for this region batch:
      const allDone = pending.length === 0; // if runSubBatches drained all
      if (allDone) return true;

      // Retry remaining sub-batches (those we didn't start or failed):
      // For simplicity, rebuild 'chunks' from 'pending' (which were not consumed),
      // but here pending is empty because we consumed. We detect failure based on counters.
      // If not all painted, we try again the whole region batch (idempotent on server).
      await new Promise(r => setTimeout(r, CONFIG.TOKEN.RETRY_BASE_DELAY_MS * attempt));
    }
    return false;
  }
